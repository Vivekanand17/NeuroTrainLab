...
function formatHyperparameterRows(modelType, hp) {
  ...
  if (modelType === "random_forest") {
    ...
    rows.push({ key: "max_depth", label: "Max depth", value: hp.max_depth == null ? "Unlimited" : hp.max_depth });
    if (hp.random_state != null) rows.push({ key: "random_state", label: "Random state", value: hp.random_state });
    if (hp.min_samples_leaf != null) rows.push({ key: "min_samples_leaf", label: "Min samples leaf", value: hp.min_samples_leaf });
  }
  return rows;
...
export default function DashboardPage({
  ...
  const activeIndex = useMemo(
    () => getPipelineActiveIndex({ eda, cleaned, training, trainingResults }),
    ...
    <>
      ...
      <section id="section-upload" className="ntl-section">
        <Card variant="glass">
          ...
          <CardBody>
            <UploadForm onUploadSuccess={onUploadSuccess} onLoadingChange={onUploadLoadingChange} />
          </CardBody>
        </Card>
      </section>
      ...
      {!eda ? (
        <div className="ntl-empty">
          ...
          <p className="ntl-empty__text">
            Upload a CSV, then choose which column to predict in the Train step. Defaults to <span className="mono">target</span> or the last column if present.
          </p>
        </div>
      ) : null}
      ...
      {eda && !eda.error ? (
        <section id="section-summary" className="ntl-section">
          <Card>
            <CardHeader>
              <CardTitle icon={<IconSparkles />}>Clean Data &amp; EDA</CardTitle>
            ...
            <CardBody>
              <div className="ntl-kv-grid">
                <div className="ntl-kv">
                  ...
                  <span className="ntl-kv__v mono">{eda.shape?.[0]} × {eda.shape?.[1]}</span>
                </div>
              ...
              <p className="ntl-label" style={{ marginTop: "1rem" }}>Data types</p>
          ...
          <Card>
            ...
            <CardBody>
              ...
              {modelType === "tree" ? (
                <div className="ntl-field-row">
                  <div>
                    <label className="ntl-label" htmlFor="max-depth">Max depth</label>
                    <input
                      id="max-depth"
                      type="number"
                      className="ntl-input"
              ...
              {modelType === "random_forest" ? (
                <div className="ntl-field-row">
                  <div>
                    <label className="ntl-label" htmlFor="n-estimators">N estimators</label>
                    <input
                      id="n-estimators"
                      type="number"
                      className="ntl-input"
                      ...
                      max={2000}
                    />
                    <span className="ntl-hint">Number of trees (default 100).</span>
                  </div>
                  <div>
                    <label className="ntl-label" htmlFor="rf-max-depth">Max depth (optional)</label>
                    <input
                      id="rf-max-depth"
                      type="number"
                      className="ntl-input"
                  ...
                  <div>
                    <label className="ntl-label" htmlFor="random-state">Random state</label>
                    <input
                      id="random-state"
                      type="number"
                      className="ntl-input"
        ...
        <section id="section-results" className="ntl-section">
          <Card>
            ...
            <CardBody>
              <div className="ntl-results-grid">
                <div className="ntl-subcard">
                  ...
                  <p className="ntl-subcard__line">
                    <span className="muted">Type</span> {MODEL_LABELS[trainingResults.model_type] || trainingResults.model_type}
                  </p>
                  {trainingResults.target_col ? (
                    <p className="ntl-subcard__line"><span className="muted">Target</span> {trainingResults.target_col}</p>
                  ) : null}
                  {formatHyperparameterRows(trainingResults.model_type, trainingResults.hyperparameters).map((row) => (
                    <p key={row.key} className="ntl-subcard__line"><span className="muted">{row.label}</span> {row.value}</p>
                  ))}
                  <p className="ntl-subcard__line">
                ...
                <div className="ntl-subcard ntl-subcard--wide">
                  ...
                  <p className="ntl-table-hint">MSE — lower is better · R² — higher is better (max 1.0)</p>
              ...
              {trainingResults.diagnostics?.messages?.length ? (
